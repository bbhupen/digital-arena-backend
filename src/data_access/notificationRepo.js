...
const selectNotificationRecordUsingNotificationType = async (data) => {
  ...
  const notificationQuery = `
    SELECT 
      ...
      b.net_total,
      b.grand_total_bill,
      GROUP_CONCAT(s.model ORDER BY s.sales_id SEPARATOR ', ') AS sale_products,
      GROUP_CONCAT(s.sale_quantity ORDER BY s.sales_id SEPARATOR ', ') AS sale_quantities,
      GROUP_CONCAT(s.total_sale_value ORDER BY s.sales_id SEPARATOR ', ') AS sale_amounts,
      COALESCE(SUM(s.total_sale_value), 0) AS sale_grand_total
    FROM ${notificationTableName} n
    INNER JOIN ${locationTableName} l 
      ON n.location_id = l.location_id
    INNER JOIN notification_type nt 
      ON n.notification_type = nt.id
    LEFT JOIN ${billTableName} b 
      ON b.bill_id = n.bill_id
    LEFT JOIN ${saleTableName} s 
      ON s.bill_no = b.bill_id
    ...
    GROUP BY 
      ...
      b.net_total,
      b.grand_total_bill
    ORDER BY n.inserted_at DESC
  `;

  const notificationResults = await executeQuery(
    notificationQuery,
    [data.notification_type] // removed extra param
  );
